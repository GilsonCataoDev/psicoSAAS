...
OBSERVAÇÕES:`},{id:`contrato-terapeutico`,title:`Contrato Terapêutico / Combinados`,description:`Honorários, faltas, cancelamento, sigilo, comunicação, emergências e proteção de dados.`,category:`formulario`,tags:[`contrato`,`ética`,`sigilo`,`LGPD`],ageGroups:[`all`],Icon:T,template:`CONTRATO TERAPÊUTICO / COMBINADOS
...
Assinatura/data:`},{id:`triagem-inicial`,title:`Entrevista de Triagem Inicial`,description:`Avaliação breve de demanda, urgência, risco, disponibilidade e adequação ao atendimento.`,category:`entrevista`,tags:[`triagem`,`avaliação inicial`,`risco`],ageGroups:[`all`],Icon:l,template:`ENTREVISTA DE TRIAGEM INICIAL
...
Próxima avaliação:`},{id:`bai-adaptado`,title:`Inventário de Ansiedade — Adaptado`,description:`Lista de sintomas físicos e cognitivos de ansiedade com escala de intensidade de 4 pontos.`,category:`escala`,tags:[`ansiedade`,`sintomas`,`BAI`],ageGroups:[`adulto`],Icon:v,template:`INVENTÁRIO DE ANSIEDADE — ADAPTADO
...
O que foi positivo:
Padrões que percebi:`},{id:`rastreio-sono`,title:`Rastreio de Sono`,description:`Avaliação do padrão de sono, comportamentos associados e impacto funcional.`,category:`registro`,tags:[`sono`,`insônia`,`higiene do sono`],ageGroups:[`adulto`,`adolescente`],Icon:f,template:`RASTREIO DE SONO
...
OBSERVAÇÕES DO(A) PROFISSIONAL:`},{id:`funcionalidade`,title:`Avaliação de Funcionalidade Global`,description:`Avaliação do funcionamento em trabalho, relacionamentos, autocuidado e autonomia.`,category:`formulario`,tags:[`funcionalidade`,`AVD`,`qualidade de vida`],ageGroups:[`adulto`],Icon:C,template:`AVALIAÇÃO DE FUNCIONALIDADE GLOBAL
...
OBSERVAÇÕES DO(A) PROFISSIONAL:`},{id:`isi`,title:`ISI — Índice de Gravidade da Insônia`,description:`Escala de 7 itens para avaliação da gravidade da insônia. Autor: Charles M. Morin. Domínio público.`,category:`escala`,tags:[`insônia`,`sono`,`ISI`,`gravidade`],ageGroups:[`adulto`],Icon:f,template:`ISI — ÍNDICE DE GRAVIDADE DA INSÔNIA
...
`),o=a.filter(e=>e.trim().endsWith(`:`)).length;function s(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`)}function c(){let e=window.open(``,`_blank`);if(!e){n.error(`Não foi possível abrir a janela de impressão.`);return}let t=a.map(e=>{if(!e.trim())return`<div style="height:8px"></div>`;if(e===e.toUpperCase()&&e.trim().length>2&&/^[A-ZÁÀÃÂÉÊÍÓÔÕÚÜÇ\s/—]+$/.test(e.trim()))return`<div style="display:flex;align-items:center;gap:8px;padding:8px 0 4px">
          <span style="font-size:8.5px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#6b7280;white-space:nowrap">${s(e.trim())}</span>
          ...
          <span style="font-size:9px;font-weight:600;color:#6b7280;padding-top:2px">${s(e.slice(0,t).trim())}</span>
          <span style="font-size:11px;color:#374151;padding-bottom:3px">${s(e.slice(t+1).trim())||``}</span></div>`:`<p style="font-size:9px;color:#9ca3af;font-style:italic;margin:2px 0">${s(e)}</p>`}).join(``),r=I[i.category];e.document.write(`<!doctype html><html><head><title>${s(i.title)}</title>
<style>
...
<div class="header">
  <p style="font-size:14px;font-weight:700;margin:0 0 2px">${s(i.title)}</p>
  <p style="font-size:9.5px;color:#6b7280;margin:0 0 4px">${s(i.description)}</p>
