      ...
      <main><article>
        ...
        <div className="mx-auto grid max-w-5xl gap-10 px-5 py-12 lg:grid-cols-[minmax(0,1fr)_240px] lg:py-16">
          <div className="min-w-0 max-w-3xl text-[1.04rem] leading-8 text-[#49443D] dark:text-neutral-200">
            ...
            {post.faq?.length ? <section className="mt-12 border-t border-[#DDE5DC] pt-8 dark:border-white/10"><h2 className="text-2xl font-bold text-[#211F1C] dark:text-neutral-50">Perguntas frequentes</h2><div className="mt-5 space-y-6">{post.faq.map(item => <div key={item.question}><h3 className="font-bold text-[#211F1C] dark:text-neutral-50">{item.question}</h3><p className="mt-2 text-base leading-7">{item.answer}</p></div>)}</div></section> : null}
