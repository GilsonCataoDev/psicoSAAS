...
const posts = [...(postsJson as BlogPost[])].sort((a, b) => b.publishedAt.localeCompare(a.publishedAt))
    ...
    <BlogShell>
      <main>
        ...
        <section className="mx-auto max-w-6xl px-5 py-14 sm:py-16" aria-labelledby="artigos-recentes">
          <div className="mb-8 flex items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[0.16em] text-sage-700 dark:text-sage-200">Biblioteca</p><h2 id="artigos-recentes" className="mt-2 text-3xl font-bold">Artigos recentes</h2></div><span className="hidden text-sm text-[#7C776B] dark:text-neutral-300 sm:block">Novos conteúdos toda semana</span></div>
          <div className="grid gap-6 md:grid-cols-2">
            {posts.map(post => (
              <article key={post.slug} className="group flex flex-col rounded-2xl border border-[#DDE5DC] bg-white p-6 shadow-card transition hover:-translate-y-0.5 hover:border-sage-300 hover:shadow-lifted dark:border-white/10 dark:bg-[#17211D] sm:p-7">
                ...
                <div className="flex flex-wrap items-center gap-3 text-xs font-semibold"><span className="rounded-full bg-sage-50 px-3 py-1.5 text-sage-800 dark:bg-sage-400/15 dark:text-sage-100">{post.category}</span><span className="inline-flex items-center gap-1.5 text-[#7C776B] dark:text-neutral-300"><CalendarDays className="h-3.5 w-3.5" /> {dateFormatter.format(new Date(`${post.publishedAt}T00:00:00Z`))}</span></div>
        ...
        <section className="border-y border-[#DDE5DC] bg-white dark:border-white/10 dark:bg-[#17211D]"><div className="mx-auto grid max-w-6xl items-center gap-7 px-5 py-12 md:grid-cols-[1fr_auto]">
          ...
          <Link to="/seguranca" className="inline-flex items-center gap-2 font-bold text-sage-700 hover:text-sage-900 dark:text-sage-200 dark:hover:text-white">Conheça nossa segurança <ArrowRight className="h-4 w-4" /></Link>
        </div></section>
      </main>
    </BlogShell>
  )
