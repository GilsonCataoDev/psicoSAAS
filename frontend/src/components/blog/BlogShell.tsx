...
export default function BlogShell({ children }: { children: ReactNode }) {
  return (
    <div className="min-h-screen bg-[#F7F8F5] text-[#211F1C]">
      <header className="sticky top-0 z-30 border-b border-[#E1E6DE] bg-[#F7F8F5]/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-4 px-5">
          ...
          <nav className="flex items-center gap-3 text-sm font-semibold sm:gap-5" aria-label="Navegação do blog">
            <Link to="/blog" className="text-sage-800 hover:text-sage-600">Conteúdos</Link>
            <Link to="/plataforma" className="hidden text-[#5F5A51] hover:text-sage-700 sm:inline">Conhecer a plataforma</Link>
      ...
      {children}
      <footer className="border-t border-[#E1E6DE] bg-white">
        <div className="mx-auto flex max-w-6xl flex-col gap-5 px-5 py-10 sm:flex-row sm:items-center sm:justify-between">
          <div><BrandLogo /><p className="mt-2 text-sm text-[#5F5A51]">Conteúdo para uma rotina clínica mais organizada.</p></div>
          <div className="flex flex-wrap gap-x-5 gap-y-2 text-sm text-[#5F5A51]">
