...
interface FisioEscalasChartProps {
  patientId: string
}

// Berg e Tinetti: score baixo = pior (lógica invertida)
// EVA-Dor: score alto = pior (lógica normal)
const SCALES = [
  {
    id: 'berg',
    label: 'Berg',
    ...
    threshMod: 45,
    threshSev: 36,
    inverted: true,  // score baixo = pior
    axisNote: '↑ melhor',
  },
  {
    id: 'tinetti',
  ...
  },
  {
    ...
    threshMod: 4,
    threshSev: 7,
    inverted: false, // score alto = pior
    axisNote: '↓ melhor',
  },
] as const
...
function severityLabelInverted(score: number, threshMod: number, threshSev: number): string {
  // Para escalas invertidas: score baixo = grave, score alto = bom
  if (score < threshSev) return 'Grave'
  if (score < threshMod) return 'Moderado'
  return 'Bom'
}

function severityLabelNormal(score: number, threshMod: number, threshSev: number): string {
  if (score >= threshSev) return 'Grave'
  if (score >= threshMod) return 'Moderado'
  return 'Leve'
}

function pointColorInverted(score: number, threshMod: number, threshSev: number): string {
  if (score < threshSev) return '#dc2626'
  if (score < threshMod) return '#d97706'
  return '#16a34a'
}

function pointColorNormal(score: number, threshMod: number, threshSev: number): string {
  if (score >= threshSev) return '#dc2626'
  if (score >= threshMod) return '#d97706'
  return '#16a34a'
}
...
export default function FisioEscalasChart({ patientId }: FisioEscalasChartProps) {
  ...
  // Eixo X comum: união de todas as datas das escalas ativas, ordenadas
  ...
  // Para escalas invertidas: pct=0% embaixo = pior, pct=100% em cima = melhor
  // Para escalas normais: pct=0% embaixo = leve, pct=100% em cima = grave
  // Em SVG y=0 é topo, então invertemos: yCoord(100) = topo, yCoord(0) = base
  ...
  for (const scale of activeScales) {
    ...
    if (scale.inverted) {
      // Para escalas invertidas, score sobe = melhora
      if (last > first) {
        trends.push({
          label: `↑ ${scale.label} em melhora`,
  ...
  return (
    <div className="rounded-2xl border border-[#DDE5DC] bg-white p-4 dark:border-white/10 dark:bg-neutral-900">
      ...
      <svg
        viewBox={`0 0 ${W} ${H}`}
        width="100%"
        aria-label="Gráfico de evolução das escalas de fisioterapia"
        role="img"
      >
        {/* Grid horizontal */}
        {gridPcts.map(pct => {
        ...
        {/* Linhas e pontos por escala */}
        {activeScales.map(scale => {
          const pts = scalePoints[scale.id]
          const plotted = pts.map(p => {
            // Para escalas invertidas: pct = (score/max)*100 — score alto vai para cima (melhor)
            // Para escalas normais: pct = (score/max)*100 — score alto vai para cima (pior)
            const pct = (p.score / scale.max) * 100
            return {
              ...p,
              pct,
          ...
          return (
            <g key={scale.id}>
              ...
              {plotted.map((p, i) => {
                ...
                const pColor = scale.inverted
                  ? pointColorInverted(p.score, scale.threshMod, scale.threshSev)
                  : pointColorNormal(p.score, scale.threshMod, scale.threshSev)
                const sevLabel = scale.inverted
                  ? severityLabelInverted(p.score, scale.threshMod, scale.threshSev)
                  : severityLabelNormal(p.score, scale.threshMod, scale.threshSev)
      ...
      <div className="mt-2 flex flex-wrap items-center gap-4 text-xs text-neutral-500">
        {activeScales.map(scale => (
          <span key={scale.id} className="flex items-center gap-1.5">
            ...
            />
            {scale.label}
            <span className="text-neutral-400">({scale.axisNote})</span>
          </span>
        ))}
