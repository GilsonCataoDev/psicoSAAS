  ...
  return (
    <nav className="lg:hidden fixed bottom-0 inset-x-0 z-50 bg-white/95 backdrop-blur-sm border-t border-neutral-100
      ...
      {items.map(({ to, icon, label }) => (
        <NavLink key={to} to={to} end={to === '/'} className="flex-1 flex flex-col items-center justify-center gap-0.5 transition-colors relative">
          {({ isActive }) => (
            <>
              {isActive && <span className="absolute top-0 left-1/2 -translate-x-1/2 w-6 h-0.5 bg-sage-500 rounded-b-full" />}
              <div className={cn('w-9 h-7 flex items-center justify-center rounded-xl transition-all relative', isActive ? 'bg-sage-50' : '')}>
              ...
              <span className={cn('text-[10px] font-medium transition-colors', isActive ? 'text-sage-600' : 'text-neutral-400')}>
                {label}
              </span>
            </>
          )}
