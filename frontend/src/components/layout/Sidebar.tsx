        ...
        {isTrialing && daysLeft !== null && daysLeft <= TRIAL_DAYS ? (
          <NavLink to="/planos" className="block hero-gradient text-white rounded-2xl p-3.5 hover:opacity-90 transition-opacity shadow-soft">
            ...
            <div className="bg-white/20 rounded-full h-1">
              <div className="bg-white rounded-full h-1 transition-all" style={{ width: `${Math.max(5, ((TRIAL_DAYS - daysLeft) / TRIAL_DAYS) * 100)}%` }} />
            </div>
          </NavLink>
        ) : subscription.status === 'active' && currentPlan ? (
          <NavLink to="/planos" className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl hover:bg-sage-50 transition-colors">
      ...
      <div className="px-3 py-3 border-t border-sage-100/70">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-neutral-50 transition-colors group">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-neutral-400 truncate">{user?.crp ? formatRegistration(user.profession, user.crp) : 'Minha conta'}</p>
          </div>
          <button
            type="button"
